...
/*   Updated: 2019/09/10 19:10:14 by dtoy             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			drawgame(t_doom *doom, t_player *player)
{
	...
	while (n < sect->npoints)
	{
		...
		{
			nyceil  = doom->sectors[sect->neighbors[n]].ceil  - player->where.z; 
       		nyfloor = doom->sectors[sect->neighbors[n]].floor - player->where.z;
		}
		...
		while (x <= endx) //отрисовка
		{
			...
			if (sect->neighbors[n] >= 0)
			{
				...
				vline(x, c.ya, cn.ya - 1, 0, w.x == w1.x || w.x == w2.x ? 0xFFFFFF : color, 0, doom->sdl);
				ytop[x] = clamp(max(c.ya, cn.ya), ytop[x], HEIGHT - 1);
                vline(x, cn.yb + 1, c.yb, 0, x == w1.x || x == w2.x ? 0xFFFFFF : color, 0, doom->sdl);
                ybot[x] = clamp(min(c.yb, cn.yb), 0, ybot[x]);
				//if (ytop[x] != 0 || ybot[x] != HEIGHT - 1)
				vline(x, ytop[x], ybot[x], 0, 0xFFFFFF, 0, doom->sdl);
			}
			else
			{
				color = 0x38cf4e;
		...
		nline(w1.x, w1.ya, w1.x, w1.yb, 0x000000, doom->sdl);
		nline(w2.x, w2.ya, w2.x, w2.yb, 0x000000, doom->sdl);
		n++;
	}
	return (0);
}