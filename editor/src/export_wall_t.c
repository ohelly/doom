...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/27 15:49:29 by ohelly            #+#    #+#             */
/*   Updated: 2019/10/27 18:21:34 by ohelly           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		output_text_wall(t_doom *doom, int count)
{
	int		i;

	i = -1;
	while (++i < count)
		out_pix_text(doom->exp->wall[i], doom->txt, doom);
}

int			count_t_wall(t_doom *doom, t_export *exp)
{
	int		count;
	int		i;
	int		j;
...
void		export_wall_tx(t_doom *doom)
{
	...
	ft_putstr_fd("W\t\t", doom->file->fd);
	count = count_t_wall(doom, doom->exp);
	ft_putnbr_fd(count, doom->file->fd);
	ft_putstr_fd("\n", doom->file->fd);
	output_text_wall(doom, count);
	ft_putstr_fd("\n", doom->file->fd);
}
